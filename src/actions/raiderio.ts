...
import { fetchCharacterRaidProgress, parseRaiderioData, resetItemsCache } from '@/lib/raiderio/raiderio-sync'
...
export async function syncAllCharactersRaiderioAction(): Promise<{
    ...
    await Promise.all(
        roster.map(async character => {
            try {
                const raiderioData = await fetchCharacterRaidProgress(character.name, character.realm)
                const parsed = await parseRaiderioData(character.name, character.realm, raiderioData)
                results.push(parsed)
            } catch (error) {
    ...
    console.log(`[Raiderio] Full Sync Completed: ${results.length} synced, ${errors.length} errors`)
 ...
 */
export async function getRosterProgressionAction(
    filter: number = 0
): Promise<{ p1Character: Awaited<ReturnType<typeof getCharactersList>>[0]; raiderIo: CharacterRaiderio | null }[]> {
    const roster = await getCharactersList()
