...
export async function checkWowAuditUpdatesAction(): Promise<{ synced: boolean; message: string }> {
    ...
    if (lastSync === null || getUnixTimestamp() - lastSync > fourHoursUnixTs) {
        ...
        return {
            synced: false,
            message: 'WowAudit is up to date (' + formaUnixTimestampToItalianDate(lastSync) + ')'
        }
    }
}
