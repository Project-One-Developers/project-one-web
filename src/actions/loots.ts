...
import { parseManualLoots, parseMrtLoots, parseRcLoots } from "@/lib/server/loot-parsers"
...
import type { SimC } from "@/shared/models/simulation.models"
import type { CharAssignmentInfo, LootAssignmentInfo } from "@/shared/types"
...
async function importRcLootInternal(
...
async function importRcLootAssignmentsInternal(
    raidSessionId: string,
    csv: string
    ...
    for (const [assignmentKey, requiredAmount] of assignmentMap.entries()) {
        ...
        for (const loot of lootsToAssign) {
            ...
            // Update the loot in sessionLoots to reflect the assignment
...
async function importMrtLootInternal(
    raidSessionId: string,
    data: string
): Promise<{ imported: number; errors: string[] }> {
    const session = await raidSessionRepo.getById(raidSessionId)
...
async function addManualLootInternal(
    raidSessionId: string,
...
async function getAssignmentInfoInternal(lootId: string): Promise<LootAssignmentInfo> {
    ...
    // Step 3: Fetch only data for eligible characters + session loots
    const [
        droptimizers,
        blizzardData,
        simcData,
    ...
    const simcByCharId = keyBy(simcData, (sc) => sc.characterId)
    ...
    const charAssignmentInfo: CharAssignmentInfo[] = filteredRoster.map((char) => {
        ...
        // Filter session loots for this character
...
async function getCharactersWithLootsByItemIdInternal(
    itemId: number
    ...
    // Stage 2: Fetch only data for eligible characters
    ...
    const res = filteredRoster.map((char) => {
        const charDroptimizers = droptimizerByCharId[char.id] ?? []
        const charBlizzard: CharacterBlizzard | null = blizzardData.get(char.id) ?? null
...
export const getLootsBySessionIdWithItem = safeAction(
    async (raidSessionId: string): Promise<LootWithItem[]> => {
        return lootRepo.getByRaidSessionIdWithItem(raidSessionId)
    }
)

export const getLootsBySessionIdsWithAssigned = safeAction(
    async (raidSessionIds: string[]): Promise<LootWithAssigned[]> => {
        return lootRepo.getByRaidSessionIdsWithAssigned(raidSessionIds)
    }
)
...
export const importRcLootCsv = safeAction(
    async (
        ...
        return importRcLootInternal(raidSessionId, csv, importAssignedCharacter)
    }
)
...
export const importRcLootAssignments = safeAction(
    async (
        ...
        return importRcLootAssignmentsInternal(raidSessionId, csv)
    }
)
...
export const importMrtLoot = safeAction(
    async (
        ...
        return importMrtLootInternal(raidSessionId, data)
    }
)
...
export const addManualLoot = safeAction(
    async (
        ...
        return addManualLootInternal(raidSessionId, manualLoots)
    }
)
