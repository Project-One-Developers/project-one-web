...
export async function syncItemsFromJson(): Promise<SyncItemsResult> {
    ...
    try {
        ...
        // Deduplicate by id
        const uniqueBosses = Array.from(new Map(allBosses.map((b) => [b.id, b])).values())
        await bossRepo.upsert(uniqueBosses)
        results.bosses.success = true
        results.bosses.count = uniqueBosses.length
    } catch (error) {
        results.bosses.error = error instanceof Error ? error.message : "Unknown error"
    ...
    try {
        ...
        for (const season of seasons) {
            const tiersetPath = path.join(resourcesPath, season, "items_to_tierset.json")
            try {
                const tierset = await loadJsonFile<ItemToTierset[]>(tiersetPath)
                allTierset.push(...tierset)
            } catch {
                logger.debug("SyncItems", `No items_to_tierset.json found for ${season}`)
            }
        }
    ...
    try {
        ...
        for (const season of seasons) {
            ...
            try {
                ...
                logger.debug("SyncItems", `No items_to_catalyst.json found for ${season}`)
            }
        }
