...
export const lootService = {
    getBySessionId: async (raidSessionId: string): Promise<Loot[]> => {
        return lootRepo.getByRaidSessionId(raidSessionId)
    },

    getBySessionIdWithItem: async (raidSessionId: string): Promise<LootWithItem[]> => {
        return lootRepo.getByRaidSessionIdWithItem(raidSessionId)
    },

    getBySessionIdWithAssigned: async (
        raidSessionId: string
    ): Promise<LootWithAssigned[]> => {
        return lootRepo.getByRaidSessionIdWithAssigned(raidSessionId)
    },

    getBySessionIdsWithAssigned: async (
    ...
    getWithItemById: async (lootId: string): Promise<LootWithItem> => {
        return lootRepo.getWithItemById(lootId)
    },
    ...
    addMany: async (raidSessionId: string, loots: NewLoot[]): Promise<void> => {
    ...
    getAssignmentInfo: async (lootId: string): Promise<LootAssignmentInfo> => {
    ...
    getCharactersWithLootsByItemId: async (
        itemId: number
        ...
        const res = filteredRoster.map((char) => {
            ...
            const lastDroptWithWeeklyChestInfo = charDroptimizers
                .filter(
                    (c) => isInCurrentWowWeek(c.simInfo.date) && c.weeklyChest.length > 0
                )
                .sort((a, b) => b.simInfo.date - a.simInfo.date)
                .at(0)
