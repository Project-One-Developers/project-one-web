...
} from "@/services/libs/blizzard-api"
import {
...
export const blizzardService = {
     ...
     */
    syncAllCharacters: async (): Promise<{
        synced: number
        errors: string[]
    }> => {
        logger.info("BlizzardService", "Start Full Sync")
        resetItemsCache()
        resetFetchedInstancesCache()
        ...
        const syncResults = await Promise.all(
            roster.map(async (character): Promise<Result<ParsedBlizzardData>> => {
                try {
                    ...
                    logger.error("BlizzardService", error)
     ...
     */
    checkAndSync: async (): Promise<{
        synced: boolean
        message: string
        result?: { synced: number; errors: string[] }
    }> => {
        logger.info("BlizzardService", "checkAndSync: checking..")
        ...
        if (lastSync === null || getUnixTimestamp() - lastSync > frequencyTS) {
            ...
            const result = await blizzardService.syncAllCharacters()
            return {
                synced: true,
     ...
     */
    importGuildMembers: async (): Promise<{
        imported: number
        skipped: number
        errors: string[]
    }> => {
        ...
        logger.info(
            "BlizzardService",
            `Found ${s(roster.members.length)} total guild members`
        )
        ...
        const existingSet = new Set(
            existingCharacters.map(
                (c) => `${c.name.toLowerCase()}-${c.realm.toLowerCase()}`
            )
        )
